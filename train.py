...
def initialize_optimizer(params, variables):
    scale = 1.0
    lrs = {
        'means3D': 0.00016 * variables['scene_radius'] / scale,
        'rgb_colors': 0.0025 / scale,
        'seg_colors': 0.0,
        'unnorm_rotations': 0.001 / scale,
        'logit_opacities': 0.05 /scale, 
        'log_scales': 0.001 / scale,
        'cam_m': 1e-4,
        'cam_c': 1e-4,
    }
    param_groups = [{'params': [v], 'name': k, 'lr': lrs[k]} for k, v in params.items()]
    return torch.optim.Adam(param_groups, lr=0.0, eps=1e-15) # eps=1e-15 to 1e-8
...
def get_loss(params, curr_data, variables, is_initial_timestep):
    ...
    # print("cam raster settings", curr_data['cam'])
    # print("im size", im.size())
    # print("cur_im size", curr_data['im'].size())
    # print("cur_im", curr_data['im'])
    # curr_data['im'] = curr_data['im'][0:3] #hack to get it to work
    ...
    if not is_initial_timestep:
        ...
        fg_count = is_fg.sum()
        prev_fg_count = variables["prev_inv_rot_fg"].shape[0] if "prev_inv_rot_fg" in variables and variables["prev_inv_rot_fg"] is not None else 0
        
        if fg_count > 0 and fg_count == prev_fg_count:
        ...
        if params['rgb_colors'].shape[0] != variables["prev_col"].shape[0]:
            old_num = variables["prev_col"].shape[0]
            new_num = params['rgb_colors'].shape[0]
            new_colors = params['rgb_colors'][old_num:].detach()
            variables["prev_col"] = torch.cat([variables["prev_col"], new_colors], dim=0)
        losses['soft_col_cons'] = l1_loss_v2(params['rgb_colors'], variables["prev_col"])

    if not is_initial_timestep and any(torch.isnan(torch.stack([v for v in losses.values()]))):
...
def initialize_per_timestep(params, variables, optimizer):
    ...
    old_fg_count = variables["neighbor_indices"].shape[0] if "neighbor_indices" in variables else 0
...
def recompute_neighbor_struct(params, variables, num_knn=20):
    is_fg = (params['seg_colors'][:, 0] > 0.5).detach()
    if is_fg.sum() < num_knn:
        return variables
    ...
    old_num_pts = variables["prev_col"].shape[0]
    new_num_pts = params['rgb_colors'].shape[0]
    if new_num_pts > old_num_pts:
        new_colors = params['rgb_colors'][old_num_pts:].detach()
        variables["prev_col"] = torch.cat([variables["prev_col"], new_colors], dim=0)
        new_pts = params['means3D'][old_num_pts:].detach()
        variables["prev_pts"] = torch.cat([variables["prev_pts"], new_pts], dim=0)
        new_rot = torch.nn.functional.normalize(params['unnorm_rotations'][old_num_pts:]).detach()
        variables["prev_rot"] = torch.cat([variables["prev_rot"], new_rot], dim=0)
...
def save_checkpoint(params, output_params, is_initial_timestep, output_dir, exp, seq, i, t, every_i=100):
    # Save checkpoint every_i iterations
    if (i + 1) % every_i == 0:
        checkpoint_dir = f"{output_dir}/{exp}/{seq}/checkpoints"
        os.makedirs(checkpoint_dir, exist_ok=True)
        checkpoint_params = params2cpu(params, is_initial_timestep)
        checkpoint_file = f"{checkpoint_dir}/timestep_{t}_iter_{i+1}.npz"
...
def train(seq, exp, data_dir, output_dir, is_save_checkpoints):
    ...
    for t in range(num_timesteps):
        ...
        min_iter = 20000 if is_initial_timestep else 12000 # Initial: 20000
        ...
        while not has_converged and i < max_iter:
            ...
            loss, variables, loss_components = get_loss(params, curr_data, variables, is_initial_timestep)
                    ...
                    if stable_loss or no_improvement:
                        ...
                        with open(timestep_log, 'a') as f:
                            f.write(f"{i},{current_loss:.6f},"
                                   f"{loss_components.get('im', 0):.6f},"
                                   f"{loss_components.get('seg', 0):.6f},"
                                   f"{loss_components.get('rigid', 0):.6f},"
                                   f"{loss_components.get('rot', 0):.6f},"
                                   f"{loss_components.get('iso', 0):.6f},"
                                   f"{loss_components.get('floor', 0):.6f},"
                                   f"{loss_components.get('bg', 0):.6f},"
                                   f"{loss_components.get('soft_col_cons', 0):.6f},"
                                   f"{psnr_value:.6f},"
            ...
            with torch.no_grad():
                ...
                if is_initial_timestep or i <= 2000:
                    # first timestep: densify for first 500-5000 iter avery 100 iter 
                    # other timesteps: densify for first 500-2000 iter every 100 iter
                ...
                if is_save_checkpoints: 
                    save_checkpoint(params, output_params, is_initial_timestep, output_dir, exp, seq, i, t)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["basketball", "boxes", "football", "juggle", "softball", "tennis", "sphere-bounce", "sphere-bounce-5"],
        help="Name of the dataset to use for training (e.g., basketball, boxes, etc.)",
    )
    parser.add_argument("--save-checkpoints", action="store_true", help="Save model checkpoints every 100 iterations")
    args = parser.parse_args()
    train(args.dataset, args.exp_name, args.data_dir, args.output_dir, args.save_checkpoints)
